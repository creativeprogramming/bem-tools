...
var BundleNode = exports.BundleNode = INHERIT(MagicNode, {
    ...
    linkNodes: function(ctx, techTargets) {
        return Q.all(Object.keys(techTargets).map(function(target) {
            ...
            if (targetNode.getDependencies)
                return Q.when(targetNode.getDependencies(ctx), function(deps) {
                    console.log(targetNode, deps);
                    for(var d = 0; d < deps.length; d++) { ctx.arch.link(
                        techTargets[deps[d]],
                        techTargets[target]);

                        console.log('linking ' + techTargets[deps[d]] + ' with ' + techTargets[target]);
                    }
                });
        }));
    },
